...
- **`setup.sh`** — Links the toolbox (`commands/`, `skills/`, `agents/`, `CLAUDE.md`) into the target config dir, merges `configs/settings/global.json` into `<dir>/settings.json`, merges `configs/mcp/global.json` (and the gitignored `global.local.json` overlay) `mcpServers` into the dir's `.claude.json` (only the `mcpServers` subtree; skipped when empty) and warns if any server's runtime `command` (e.g. `npx`, `uvx`) is missing from `PATH`, and symlinks the CLI to `~/.local/bin/`. Runs `validate.sh` first. Flags: `--unlink` (remove links, restore backups), `--skip-validation` (bypass agent validation). On WSL, also runs `configs/devcontainer/wsl-setup.sh`.
  - **Target config dir / multiple accounts:** defaults to the active account — `$CLAUDE_CONFIG_DIR` if set, else `~/.claude`. The big config file is `~/.claude.json` for the default account but `<dir>/.claude.json` for any account selected via `CLAUDE_CONFIG_DIR`. Set `TOOLBOX_ACCOUNTS` to a colon-separated list of config dirs to sync several accounts in one run, e.g. `TOOLBOX_ACCOUNTS="$HOME/.claude:$HOME/.claude-work" ./setup.sh`. The CLI symlink and WSL setup are account-independent and dedupe across iterations.
